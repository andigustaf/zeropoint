    ...
    >
      <Box w="full" maxW={"lg"} px={6}>
        ...
        <Flex bg="white" justifyContent="space-between" p={2} paddingX={4} roundedTop={"lg"} >
          ...
          <Text><Button size={'sm'} variant={'link'}>View Log</Button></Text>
        </Flex>
          <TableContainer>
                ...
                <Tbody>
                  {
                    attendances.map((attendance, attendanceIndex) => (
                      <Tr key={attendanceIndex}>
                        <Td> { formatTimestamp(attendance.timestamp.seconds) } </Td>
                        <Td textTransform={'capitalize'}>{ formatAttendanceType(attendance.type) }</Td>
                        <Td>
                          <Box textAlign="right">
                              <ChevronRightIcon />
                          </Box>
